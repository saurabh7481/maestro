...
### 3.5 OpenCode CLI (the fifth adapter — server-backed)

opencode differs from the other four in one structural way: it is a
**server plus thin clients** (its TUI and IDE plugins are clients of an
HTTP server), and its provider credentials/model catalog live in that
server's world. Maestro therefore runs one lazy `opencode serve`
sidecar and is itself just another client — full rationale, lifecycle,
and provider-management UX in
[`OPENCODE_INTEGRATION.md`](./OPENCODE_INTEGRATION.md); this section is
the protocol reference.

- **Headless turns:** `opencode run --format json [prompt]` → NDJSON
  `{type, timestamp, sessionID, part}` lines. Types: `step_start`,
  `text`, `reasoning`, `tool_use`, `step_finish`. Text/reasoning arrive
  only as finished blocks (`Streaming::Blocks` — measured, fixture 02);
  tool parts arrive once with terminal state (`state.status:
  completed|error`, input/output/metadata inside).
- **Attach:** `run --attach http://127.0.0.1:<port> -p <password> --dir
  <worktree>` reuses Maestro's warm sidecar instead of cold-booting an
  internal server per message. In attach mode `--dir` means "path on the
  remote server" and must always be passed explicitly.
- **Sessions:** `--session <id>` / `-c` / `--fork`; list via
  `session list --format json` (rows carry `directory` — worktree scoping
  is direct); transcript replay via `export <id>` (`{info,
  messages[].parts}`). Titles are generated by opencode's small model.
- **Model/effort:** `-m provider/model`; reasoning effort is `--variant`
  (separate flag, not baked into the model id). Variant sets aren't
  enumerable anywhere public, so no variant picker ships.
- **Permissions:** config-driven (`opencode.json` permission rules) +
  `--auto`. A tool hitting an "ask" rule without `--auto` is refused
  (`state.status: "error"`, verbatim rejection message) **and terminates
  the run** — the model is never re-consulted. Hence
  `ManualGate::ExternalConfig`.
- **Usage:** every `step_finish` carries `tokens {input, output,
  reasoning, cache{read, write}}` + `cost`; the adapter sums them into
  the turn result.
- **Provider management (unique to this CLI):** auth lives in
  `~/.local/share/opencode/auth.json`, written through the server API —
  `GET /provider` (+`connected[]`), declarative `GET /provider/auth`
  methods/prompts, `PUT`/`DELETE /auth/{id}`, OAuth via
  `/provider/{id}/oauth/authorize|callback`. Responses contain key
  material; the command layer projects before IPC. The sidecar binds
  loopback on an ephemeral port behind a per-boot random password.
- **Version floor:** verified against 1.18.x; older builds fail at spawn
  on unknown flags rather than silently misbehaving (CHECKLIST sweep).
